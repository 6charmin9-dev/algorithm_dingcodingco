...
def get_all_ways_of_theater_seat(total_count, fixed_seat_array):
    memo = {0:0, 1:1, 2:2}
    fixed_seat_array.sort()
    all_ways = 1
    start_seat = 1
    for fixed_seat in fixed_seat_array:
        segment_length = fixed_seat - start_seat
        # print('segment_length:', segment_length)
        all_ways *= fibo(segment_length, memo)
        start_seat = fixed_seat + 1
    segment_length = total_count - start_seat + 1
    # print('segment_length:', segment_length)
    all_ways *= fibo(segment_length, memo)
    # print('memo:', memo)
    return all_ways
